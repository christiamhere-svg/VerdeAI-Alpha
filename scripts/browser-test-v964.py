...
RESULT=OUT/'v964_browser_test_results.json'
html=(ROOT/'index.html').read_text(encoding='utf-8')
css=(ROOT/'styles/main.v9.6.4.css').read_text(encoding='utf-8')
config=(ROOT/'config.v9.6.4.js').read_text(encoding='utf-8')
app=(ROOT/'js/app.v9.6.4.js').read_text(encoding='utf-8')
...
html=html.replace('<link rel="stylesheet" href="styles/main.v9.6.4.css" />',f'<style>{css}</style>')
html=html.replace('<script src="config.v9.6.4.js"></script>',f'<script>{config}</script>')
html=html.replace('<script defer src="js/app.v9.6.4.js"></script>',f'<script>{app}</script>')
...
with sync_playwright() as pw:
    ...
    try:
        ...
        assert 'Low-Maintenance Haven inspiration' in text
        ...
        p.screenshot(path=str(OUT/'v964_tester_page_android.png'), full_page=True)
        if errors: raise AssertionError(errors)
        results.append({'case':'v9.6.4-tester-page-alignment','status':'passed','clean_finished_result':True,'current_photo_and_selection':True,'copy_aligned':True,'direct_feedback':True,'safe_ai_state':True,'overflow':overflow})
    except Exception as exc:
        results.append({'case':'v9.6.4-tester-page-alignment','status':'failed','error':str(exc),'trace':traceback.format_exc(),'browser_errors':errors})
    finally:
