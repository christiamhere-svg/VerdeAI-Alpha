...
RESULT_FILE = ROOT / 'evidence' / 'v911_browser_test_results.json'

html = (ROOT / 'index.html').read_text()
css = (ROOT / 'styles/main.v9.1.1.css').read_text()
config = (ROOT / 'config.v9.1.1.js').read_text()
app = (ROOT / 'js/app.v9.1.1.js').read_text()
html = html.replace('<link rel="stylesheet" href="styles/main.v9.1.1.css" />', f'<style>{css}</style>')
html = html.replace('<script src="config.v9.1.1.js"></script>', f'<script>{config}</script>')
html = html.replace('<script type="module" src="js/app.v9.1.1.js"></script>', f'<script>{app}</script>')
...
with sync_playwright() as p:
    ...
    try:
        ...
        assert 'Build v9.1.1' in page.locator('.build-pill').inner_text()
        analyse(page)
        ...
        assert visual.get_attribute('data-rendered-build') == '9.1.1'
        ...
        open_shot = OUT / 'v911_calibration_active_mobile.png'
        ...
        clean_shot = OUT / 'v911_clean_finished_concept_mobile.png'
        ...
        restored_shot = OUT / 'v911_restored_after_refresh_mobile.png'
        ...
        results.append({
            'case':'v9.1.1-dedicated-host-and-persistence', 'status':'passed',
            'physical_phone_evidence':'live v9.1 Android screenshots confirm dragging, Done, and marker 5; legacy content remained visible',
        ...
        results.append({'case':'v9.1.1-dedicated-host-and-persistence','status':'failed','error':str(e),'trace':traceback.format_exc(),'browser_errors':errors})
    finally:
