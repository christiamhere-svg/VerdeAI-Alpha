...
RESULT=OUT/'v965_browser_test_results.json'
html=(ROOT/'index.html').read_text(encoding='utf-8')
css=(ROOT/'styles/main.v9.6.5.css').read_text(encoding='utf-8')
config=(ROOT/'config.v9.6.5.js').read_text(encoding='utf-8')
app=(ROOT/'js/app.v9.6.5.js').read_text(encoding='utf-8')
...
html=html.replace('<link rel="stylesheet" href="styles/main.v9.6.5.css" />',f'<style>{css}</style>')
html=html.replace('<script src="config.v9.6.5.js"></script>',f'<script>{config}</script>')
html=html.replace('<script defer src="js/app.v9.6.5.js"></script>',f'<script>{app}</script>')
...
with sync_playwright() as pw:
    ...
    try:
        ...
        assert 'build v9.6.5' in visible.lower()
        ...
        p.screenshot(path=str(OUT/'v965_android_entry.png'), full_page=True)
    ...
    try:
        ...
        overflow=p.evaluate('document.documentElement.scrollWidth-document.documentElement.clientWidth')
        assert overflow <= 2, overflow
        p.screenshot(path=str(OUT/'v965_desktop_entry.png'), full_page=True)
        if errors: raise AssertionError(errors)
        results.append({'case':'desktop-first-entry','status':'passed','overflow':overflow})
    except Exception as exc:
