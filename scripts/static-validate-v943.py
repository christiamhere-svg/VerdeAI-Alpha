...
html=(r/'index.html').read_text();js=(r/'js/app.v9.4.3.js').read_text();css=(r/'styles/main.v9.4.3.css').read_text();cfg=(r/'config.v9.4.3.js').read_text()
checks={
'version': 'Build v9.4.3' in html and 'const BUILD_VERSION = "9.4.3"' in js and 'version: "9.4.3"' in cfg,
...
out={'version':'9.4.3','status':'passed' if all(checks.values()) else 'failed','checks':checks};(r/'static-validation-v943.json').write_text(json.dumps(out,indent=2));print(json.dumps(out,indent=2));sys.exit(0 if all(checks.values()) else 1)
