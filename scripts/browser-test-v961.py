...
RESULT=OUT/'v961_browser_test_results.json'
html=(ROOT/'index.html').read_text(encoding='utf-8')
css=(ROOT/'styles/main.v9.6.1.css').read_text(encoding='utf-8')
config=(ROOT/'config.v9.6.1.js').read_text(encoding='utf-8')
app=(ROOT/'js/app.v9.6.1.js').read_text(encoding='utf-8')
...
html=html.replace('<link rel="stylesheet" href="styles/main.v9.6.1.css" />',f'<style>{css}</style>')
html=html.replace('<script src="config.v9.6.1.js"></script>',f'<script>{config}</script>')
html=html.replace('<script defer src="js/app.v9.6.1.js"></script>',f'<script>{app}</script>')
...
with sync_playwright() as pw:
    ...
    try:
        ...
        assert 'Build v9.6.1' in p.locator('body').inner_text()
        ...
        assert cards.locator('.hybrid-card-inspiration .hybrid-reference.is-compact').count()==6
        assert cards.locator('.hybrid-section-copy').count()==12
        assert cards.locator('.hybrid-card-explanation').count()==6
        assert cards.locator('.honest-placement-map').count()==6
        assert cards.locator('.hybrid-reference img').count()==6
        labels=' '.join(cards.locator('.hybrid-section-copy').all_inner_texts()).lower()
        assert labels.count('real-world inspiration')==6
        assert labels.count('not your property')==6
        ...
        assert host.locator('.hybrid-concept-grid').evaluate("el => getComputedStyle(el).gridTemplateColumns.split(' ').length") == 1
        ...
        assert 'not your property' in text and 'not an exact render' in text and 'inspiration only' in text
        ...
        board.screenshot(path=str(OUT/'v961_demo_board_android.png'))
        host.screenshot(path=str(OUT/'v961_demo_fullsize_android.png'))
        if errors: raise AssertionError(errors)
        results.append({'case':'v9.6.1-honest-hybrid-board-layout-repair','status':'passed','six_hybrid_futures':True,'map_reference_separation':True,'recommended_selected_independent':True,'calibration_preserved':True,'safe_ai_state':True,'overflow':overflow})
    except Exception as exc:
        results.append({'case':'v9.6.1-honest-hybrid-board-layout-repair','status':'failed','error':str(exc),'trace':traceback.format_exc(),'browser_errors':errors})
    finally:
