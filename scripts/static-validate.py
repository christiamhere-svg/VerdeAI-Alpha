...
js = (root / 'js/app.v9.3.2.js').read_text(encoding='utf-8')
css = (root / 'styles/main.v9.3.2.css').read_text(encoding='utf-8')
config = (root / 'config.v9.3.2.js').read_text(encoding='utf-8')

checks = {
  'v932_visible_and_stored': 'Build v9.3.2' in html and 'const BUILD_VERSION = "9.3.2"' in js and 'version: "9.3.2"' in config,
  'unique_assets': all(x in html for x in ['styles/main.v9.3.2.css','config.v9.3.2.js','js/app.v9.3.2.js']),
  'future_panel_and_names': all(x in html+js for x in ['futureSelectionPanel','Other possible futures','Feature Garden','Low-Maintenance Haven','Wildlife Haven','Gathering Space','Food Garden','Maker / Workshop Yard']),
  ...
  'botanical_types': all(x in js for x in ['groundcover','flower-shrub','grass','screen','tree','herb','edible-bed','fern','perennial']),
  'grounding_and_perspective': all(x in js+css for x in ['plant-contact-shadow','perspective','.plant-ground-base','.plant-rear-layer','.plant-mid-layer','.plant-front-layer']),
  ...
  'future_text_sync': 'futureCompositionText' in js and '${profile.pattern}: ${constraintLabel' not in js,
  ...
  'photo_clear': all(x in css for x in ['.concept-depth-wash{display:none', 'filter:none!important;opacity:1!important', '.site-ground-veil{fill:rgba(44,69,42,.008)']),
  ...
  'browser_evidence': (root/'evidence/v932_browser_test_results.json').exists(),
  'evidence_multiple_futures': all((root/'evidence'/f'v932_{x}_overlay_mobile.png').exists() for x in ['wildlife','belonging','minimal']),
...
result = {
  ...
  'scope': 'Botanical Overlay Realism Pass',
  'checks': checks,
}
