...
RESULT=OUT/'v944_browser_test_results.json'
html=(ROOT/'index.html').read_text(encoding='utf-8')
css=(ROOT/'styles/main.v9.4.4.css').read_text(encoding='utf-8')
config=(ROOT/'config.v9.4.4.js').read_text(encoding='utf-8')
app=(ROOT/'js/app.v9.4.4.js').read_text(encoding='utf-8')
...
html=html.replace('<link rel="stylesheet" href="styles/main.v9.4.4.css" />',f'<style>{css}</style>')
html=html.replace('<script src="config.v9.4.4.js"></script>',f'<script>{config}</script>')
html=html.replace('<script defer src="js/app.v9.4.4.js"></script>',f'<script>{app}</script>')
...
with sync_playwright() as pw:
    ...
    try:
        ...
        assert 'Build v9.4.4' in p.locator('body').inner_text()
        ...
        assert board.locator('.future-visual-signature').count()==6
        signature_text=' '.join(board.locator('.future-visual-signature').all_inner_texts()).lower()
        for expected in ['focal garden bed','calm low-care structure','layered wildlife habitat','outdoor gathering room','productive edible beds','practical work zone']:
            assert expected in signature_text, expected
        ...
        board.screenshot(path=str(OUT/'v944_demo_board_android.png'))
        host.screenshot(path=str(OUT/'v944_demo_fullsize_android.png'))
        if errors: raise AssertionError(errors)
        results.append({'case':'v9.4.4-six-futures-contrast-pass','status':'passed','real_demo_photo':True,'six_futures':True,'photo_specific_calibration':True,'recommended_selected_independent':True,'finished_access_guide_hidden':True,'marker_5_visible':True,'safe_ai_state':True,'overflow':overflow})
    except Exception as exc:
        results.append({'case':'v9.4.4-six-futures-contrast-pass','status':'failed','error':str(exc),'trace':traceback.format_exc(),'browser_errors':errors})
    finally:
