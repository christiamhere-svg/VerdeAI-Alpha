...
PHOTO = ROOT / 'evidence/v94_test_property_clean.jpg'
RESULT = OUT / 'v94_browser_test_results.json'

html = (ROOT / 'index.html').read_text(encoding='utf-8')
css = (ROOT / 'styles/main.v9.4.css').read_text(encoding='utf-8')
config = (ROOT / 'config.v9.4.js').read_text(encoding='utf-8')
app = (ROOT / 'js/app.v9.4.js').read_text(encoding='utf-8')
html = html.replace('<link rel="stylesheet" href="styles/main.v9.4.css" />', f'<style>{css}</style>')
html = html.replace('<script src="config.v9.4.js"></script>', f'<script>{config}</script>')
html = html.replace('<script type="module" src="js/app.v9.4.js"></script>', f'<script>{app}</script>')
...
with sync_playwright() as pw:
    ...
    try:
        ...
        assert 'BUILD V9.4' in p.locator('body').inner_text().upper()
        prepare_result(p)

        board = p.locator('#possibilitiesBoard')
        assert board.is_visible()
        assert 'WHAT COULD YOUR PROPERTY BECOME?' in board.inner_text().upper()
        today_anchor = board.locator('#dashboardBoardToday')
        assert today_anchor.locator('img').count() == 1
        assert today_anchor.locator('.concept-overlay-svg').count() == 0
        assert board.locator('#dashboardRecommendation h2').count() == 1
        assert board.locator('.board-compass-row').count() == 6
        assert board.locator('#dashboardEvolution .evolution-step').count() == 5
        assert board.locator('#dashboardEvolution .evolution-step').first.locator('.evolution-concept').count() == 0
        assert board.locator('#dashboardEvolution .evolution-step').nth(4).locator('.evolution-concept').count() == 1
        ...
        for fid in ['wildlife', 'belonging', 'minimal']:
            ...
            shot = OUT / f'v94_{fid}_overlay_mobile.png'
        ...
        recommended_id = p.evaluate('state.recommendedFutureId')
        alternate_id = next(fid for fid in ['belonging','minimal','wildlife','gathering','productive','maker'] if fid != recommended_id)
        panel.locator(f'[data-dashboard-future="{alternate_id}"]').click(); p.wait_for_timeout(250)
        state_data = p.evaluate('({selected: state.selectedFutureId, recommended: state.recommendedFutureId})')
        assert state_data['selected'] == alternate_id
        assert state_data['recommended'] == recommended_id
        assert state_data['recommended'] != state_data['selected']
        assert p.locator('#futureSelectionStatus').inner_text().strip()

        # Visual evidence of the coherent board on Android.
        p.set_viewport_size({'width': 390, 'height': 844}); p.wait_for_timeout(2400)
        first_card = panel.locator('[data-dashboard-future]').first
        card_dims = first_card.evaluate('(el) => ({scroll:el.scrollWidth, client:el.clientWidth})')
        assert card_dims['scroll'] <= card_dims['client'] + 1, card_dims
        board.scroll_into_view_if_needed(); p.wait_for_timeout(120)
        p.add_style_tag(content='.skip-link,.public-tabs,.toast{display:none!important}')
        board.screenshot(path=str(OUT / 'v94_possibilities_board_android.png'))
        host.screenshot(path=str(OUT / 'v94_full_size_selected_android.png'))
        assert overflow(p) <= 2

        # Desktop board evidence.
        p.set_viewport_size({'width': 1440, 'height': 1000}); p.wait_for_timeout(300)
        board.screenshot(path=str(OUT / 'v94_possibilities_board_desktop.png'))
        ...
        results.append({
            'case': 'v9.4-property-possibilities-board',
            'status': 'passed',
            'property_today_anchor': True,
            'six_futures_visible_together': True,
            'recommendation_compass_next_steps_evolution': True,
            ...
            'safe_ai_state': True,
            'viewports': widths,
            'screenshots': captures + [str(OUT / 'v94_possibilities_board_android.png'), str(OUT / 'v94_possibilities_board_desktop.png'), str(OUT / 'v94_full_size_selected_android.png')],
            'browser': 'Chromium inline harness',
            'physical_android_gate': 'required before invited testers'
        })
    except Exception as exc:
        results.append({'case': 'v9.4-property-possibilities-board', 'status': 'failed', 'error': str(exc), 'trace': traceback.format_exc(), 'browser_errors': errors})
    finally:
