...
RESULT=OUT/'v942_browser_test_results.json'
html=(ROOT/'index.html').read_text(encoding='utf-8')
css=(ROOT/'styles/main.v9.4.2.css').read_text(encoding='utf-8')
config=(ROOT/'config.v9.4.2.js').read_text(encoding='utf-8')
app=(ROOT/'js/app.v9.4.2.js').read_text(encoding='utf-8')
...
html=html.replace('<link rel="stylesheet" href="styles/main.v9.4.2.css" />',f'<style>{css}</style>')
html=html.replace('<script src="config.v9.4.2.js"></script>',f'<script>{config}</script>')
html=html.replace('<script defer src="js/app.v9.4.2.js"></script>',f'<script>{app}</script>')
...
with sync_playwright() as pw:
    ...
    try:
        ...
        assert 'Build v9.4.2' in p.locator('body').inner_text()
        ...
        board.screenshot(path=str(OUT/'v942_demo_board_android.png'))
        host.screenshot(path=str(OUT/'v942_demo_fullsize_android.png'))
        if errors: raise AssertionError(errors)
        results.append({'case':'v9.4.2-demo-grounding-hotfix','status':'passed','real_demo_photo':True,'six_futures':True,'photo_specific_calibration':True,'recommended_selected_independent':True,'finished_access_guide_hidden':True,'marker_5_visible':True,'safe_ai_state':True,'overflow':overflow})
    except Exception as exc:
        results.append({'case':'v9.4.2-demo-grounding-hotfix','status':'failed','error':str(exc),'trace':traceback.format_exc(),'browser_errors':errors})
    finally:
