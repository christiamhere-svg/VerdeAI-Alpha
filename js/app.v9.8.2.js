...
function runAnalysis(options = {}) {
  ...
  if (!options.quiet && state.photoDataUrl && !state.demoMode && !state.selfTestMode) {
    window.setTimeout(() => {
      ...
      toast("Six directions ready · next, download the manual image package");
    }, 80);
  }
}
...
function renderManualRenderMode() {
  ...
  if (status) {
    ...
    status.innerHTML = `<strong>${count} of ${FUTURES.length}</strong><span>${complete ? "Same-property futures imported — board complete" : state.manualRenderPackageCreatedAt ? "Package downloaded — import each finished image" : "Ready to download the one-file request package"}</span>`;
