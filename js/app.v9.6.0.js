...
function conceptVisualHtml(mode = normaliseVisualMode(), options = {}) {
  ...
  return `<div class="photo-first-visual-shell mode-${mode}" data-clean-visual-panel="v9.6">${switcher}${calibration}<div class="hybrid-concept-grid">
    <section class="hybrid-concept-property"><div class="hybrid-panel-heading"><b>Your property · where it could go</b><span>Calibrated placement map</span></div><div class="photo-concept-stage mode-${mode}${state.demoMode ? " demo-natural-ratio" : ""}${noPhoto} is-finished" style="${stageBackground}; --future-color:${future.color}">${photoLayer}${honestPlacementMapHtml(future)}<span class="visual-mode-chip">Placement map · not a render</span></div></section>
    <section class="hybrid-concept-reference"><div class="hybrid-panel-heading"><b>Real-world inspiration · what it could feel like</b><span>Not your property</span></div>${hybridReferenceHtml(future)}<div class="hybrid-borrow-box"><b>Borrow the idea, not the photograph</b><p>${escapeHtml(ref.idea)}</p></div></section>
  </div>${context}<div class="honest-hybrid-note"><b>Honest Hybrid Future</b><span>VerdeAI shows the believable location on your exact photo and a separate real reference for the atmosphere. It does not pretend the reference is an exact render of your property.</span></div></div>`;
}
...
function hybridReferenceHtml(future, compact = false) {
  ...
  return `<figure class="hybrid-reference${compact ? ' is-compact' : ''}">
    ...
    <figcaption><b>Real-world inspiration</b><span>Not your property · not an exact render</span>${compact ? '' : `<p>${escapeHtml(ref.idea)}</p><small>${escapeHtml(ref.credit)}</small>`}</figcaption>
  </figure>`;
}
...
function dashboardFutureCardHtml(future, index) {
  ...
  return `<article class="dashboard-future-card honest-hybrid-card future-${future.id} ${isSelected ? "active" : ""}" data-dashboard-future="${future.id}" style="--future-color:${future.color}; --overlay-tint:${future.tint}" role="button" tabindex="0" aria-pressed="${isSelected}" aria-label="Select ${escapeHtml(future.title)} future">
    ...
    <div class="hybrid-card-visual">
      <div class="hybrid-property-mini">${photo}${honestPlacementMapHtml(future, true)}<span class="hybrid-half-label">Your property · where</span></div>
      ${hybridReferenceHtml(future, true)}
      <span class="future-visual-signature"><b>${escapeHtml(signature.label)}</b><small>${escapeHtml(signature.detail)}</small></span>
    </div>
