...
function dashboardFutureCardHtml(future, index) {
  ...
  return `<article class="dashboard-future-card future-photo-option future-${future.id} ${isSelected ? "active" : ""}" data-dashboard-future="${future.id}" style="--future-color:${future.color}; --overlay-tint:${future.tint}" role="button" tabindex="0" aria-pressed="${isSelected}" aria-label="Select ${escapeHtml(future.title)} future">
    ...
    <div class="dashboard-future-visual future-photo-preview" aria-label="${escapeHtml(future.title)} on this property photo">${photo}${overlayHtml(future, { showTrust: false, mode: "selected" })}<span class="concept-preview-note">Same property</span><span class="future-visual-signature"><b>${escapeHtml(signature.label)}</b><small>${escapeHtml(signature.detail)}</small></span></div>
    <div class="dashboard-future-copy">
      ...
      <button type="button" class="secondary view-future-photo" data-view-future="${future.id}">Open ${escapeHtml(future.title)}</button>
    </div>
  </article>`;
}
