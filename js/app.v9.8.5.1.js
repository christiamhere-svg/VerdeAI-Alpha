...
function smartNextPlan() {
  ...
  const hasHandoffAction = (state.history || []).some((item) => /saved|feedback|copied|export|share/i.test(`${item.title} ${item.detail}`));
  if (!hasPhoto) {
    return { action: "photo", label: "Upload one property photo", detail: "Start with one real photo. VerdeAI will prepare six same-property directions and a manual render package." };
  }
  ...
  if (!state.analysisComplete) {
    return { action: "analyse", label: "Analyse Property", detail: "Create the six future directions, then continue straight to the no-cost manual image package." };
  }
  if (!hasHandoffAction) {
...
function openManualImages() {
  ...
  if (!manualRenderReady()) {
    ...
    toast(state.photoDataUrl ? "Tap one clue to create the six directions" : "Choose one property photo first");
    return;
  }
  activateTab("dashboard");
  window.setTimeout(() => {
  ...
  toast("Six-image workflow opened");
}

function renderManualJourney() {
  const count = manualRenderCount();
  const hasPhoto = Boolean(state.photoDataUrl);
  const analysed = Boolean(state.analysisComplete && hasPhoto && !state.demoMode && !state.selfTestMode);
  const packaged = Boolean(state.manualRenderPackageCreatedAt);
  let step = 1;
  ...
  if (hasPhoto && !analysed) {
    ...
    detail = "One clue is enough. VerdeAI will create the six directions automatically.";
    progress = 38;
  } else if (analysed && !packaged) {
    step = 3;
    title = "Download the six-image package.";
    detail = "It contains this exact photo, preservation rules, and six labelled prompts.";
    progress = 64;
  } else if (analysed && packaged && count < FUTURES.length) {
    step = 4;
    title = "Add the finished images.";
    detail = `${count} of ${FUTURES.length} images added. Match each ChatGPT result to its labelled future.`;
    progress = 76 + Math.round((count / FUTURES.length) * 22);
  } else if (analysed && count === FUTURES.length) {
    step = 4;
    title = "Your six-futures board is ready.";
    detail = "All six same-property images are in place.";
    progress = 100;
  }

  const guide = $("manualJourneyStatus");
  if (guide) guide.dataset.step = String(step);
  setText("manualJourneyStep", count === FUTURES.length ? "Complete" : `Step ${step} of 4`);
  ...
  if (countBadge) countBadge.textContent = `${count}/${FUTURES.length}`;
  const imagesTab = $("manualImagesTab");
  if (imagesTab) {
    imagesTab.classList.toggle("is-ready", analysed);
    imagesTab.classList.toggle("is-complete", count === FUTURES.length);
    imagesTab.setAttribute("aria-label", `Open images workflow, ${count} of ${FUTURES.length} images added`);
  }
}
