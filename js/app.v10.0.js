...
function openWholeBoardOverlay(profile, selected, recommended, ranked) {
  ...
  canvas.innerHTML = `<div class="whole-board-poster whole-board-poster-full">${wholeBoardPosterHtml(profile, selected, recommended, ranked)}</div>`;
...
function renderWholeBoard(profile, selected, recommended, ranked, boardReady) {
  ...
  if (!wholeBoardResizeBound) {
    ...
    window.addEventListener('resize', () => window.requestAnimationFrame(syncWholeBoardScale));
