...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "VerdeAI v3.4 Mock Backend", mode: "mock", version: "3.4.0" });
});
...
app.post("/api/analyse", (req, res) => {
  ...
  res.json({
    ...
    summary: `Mock analysis selected ${selectedFuture} for a ${propertyType} with ${preference} direction and ${constraint} problem. v3.4 treats the image as an overlay base and uses human clues until real vision is connected.`,
    selectedFuture,
    confidence: 88
  });
});
...
app.post("/api/render", (req, res) => {
  ...
  res.json({
    ...
    message: "Real image generation is not connected in v3.4. This endpoint returns a provider-ready mock contract for future backend proxy rendering.",
...
app.listen(port, () => {
  console.log(`VerdeAI v3.4 mock backend running on http://localhost:${port}`);
});
