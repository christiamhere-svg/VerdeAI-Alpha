...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "VerdeAI v6.0 Mock Backend", mode: "safe-render-proxy-scaffold", version: "6.0.0", realRenderingEnabled: process.env.VERDEAI_REAL_RENDERING_ENABLED === "true" });
});

app.get("/api/render/providers", (_req, res) => {
  res.json({ ok: true, version: "6.0.0", providers: renderProvidersStatus(), warning: "Provider keys must stay server-side. Paid providers are disabled unless VERDEAI_REAL_RENDERING_ENABLED=true and cost is confirmed." });
});

app.post("/api/render/estimate", (req, res) => {
  const estimate = validateRenderRequest(req.body || {});
  res.json({ ok: true, version: "6.0.0", provider: estimate.provider, futureId: estimate.futureId, requestedCount: estimate.count, estimatedCostUsd: estimate.estimatedCostUsd, confirmationRequired: estimate.realProviderRequested, allowed: estimate.allowed, blockReason: estimate.blockReason });
});
...
app.post("/api/analyse", (req, res) => {
  ...
  res.json({
    ...
    summary: `Mock analysis selected ${selectedFuture} for a ${propertyType} with ${preference} direction and ${constraint} problem. v6.0 treats the image as an overlay base and uses human clues until real vision is connected.`,
    selectedFuture,
    confidence: 88
  });
});
...
app.post("/api/futures", (req, res) => {
  ...
  res.json({ ok: true, mode: "mock-future-composer", version: "6.0.0", selectedFuture, futures });
});

app.post("/api/render", async (req, res) => {
  try {
    const response = await handleRenderRequest(req.body || {});
    res.json({ ...response, version: "6.0.0" });
  } catch (error) {
    res.status(500).json({ ok: false, version: "6.0.0", mode: "render-proxy-error", message: error.message, safeFallback: "Use concept board result; no paid render was completed." });
  }
});

app.post("/api/report", (req, res) => {
  res.json({ ok: true, mode: "mock-report", version: "6.0.0", report: req.body?.report || "No report supplied.", generatedAt: new Date().toISOString() });
});

app.listen(port, () => {
  console.log(`VerdeAI v6.0 mock backend running on http://localhost:${port}`);
});
