...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "VerdeAI v2.5 Mock Backend", mode: "mock", version: "2.5.0" });
});
...
app.post("/api/analyse", (req, res) => {
  ...
  res.json({
    ...
    summary: `Mock analysis selected ${selectedFuture} for a ${propertyType} with ${preference} direction and ${constraint} problem. v2.5 treats the image as an overlay base and uses human clues until real vision is connected.`,
    selectedFuture,
    confidence: 88
  });
});
...
app.listen(port, () => {
  console.log(`VerdeAI v2.5 mock backend running on http://localhost:${port}`);
});
