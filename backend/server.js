...
app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "VerdeAI v3.2 Mock Backend", mode: "mock", version: "3.2.0" });
});
...
app.post("/api/analyse", (req, res) => {
  ...
  res.json({
    ...
    summary: `Mock analysis selected ${selectedFuture} for a ${propertyType} with ${preference} direction and ${constraint} problem. v3.2 treats the image as an overlay base and uses human clues until real vision is connected.`,
    selectedFuture,
    confidence: 88
  });
});
...
app.listen(port, () => {
  console.log(`VerdeAI v3.2 mock backend running on http://localhost:${port}`);
});
